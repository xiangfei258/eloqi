...
| Windows | `windows/amd64` full build, vet, tests, and golangci-lint passed locally; focused `386` tests and `arm64` platform/main-program cross-build passed. The downloaded `v0.1.0-rc.2` archive also passed a partial Windows 11 physical run: real F8 hotkey and microphone input, six non-empty WAV sessions through a loopback WAV-checking endpoint, zero-delay/long-hold behavior, clipboard and automatic paste, overlay focus/click-through/Alt+Tab, and clean shutdown/restart. | Real-model transcription, ordinary-user permissions, Unicode/emoji/newline output, error/TUI paths, high-DPI/multi-display visuals, and microphone release to a second application. |
