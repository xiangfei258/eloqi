...
- [ ] 定义 recorder 接口：启动、读音频、停止（返回剩余数据）。
- [ ] 定义 ASR 接口：连接、发送音频、结果回调、最终文本、关闭。
- [ ] 定义 clipboard 接口：读取、写入。
- [ ] 定义 autotype 接口：把文本注入当前焦点窗口。
- [ ] 定义 hotkey 接口：注册/注销热键、事件回调（按下/抬起边沿）。
- [ ] 每个接口配一个 mock 实现，供后续测试用。
