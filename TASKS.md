...
- [ ] macOS Intel。
- [ ] macOS Apple Silicon。
- [ ] Windows（2026-08-15 已完成下载版 rc.2 的部分真机回归：实体 F8/麦克风、0ms 与长按、防重复、中文/emoji/换行剪贴板与自动上屏、录音中 Escape 取消、首请求 503 后 R 新会话重试、overlay 点击穿透/Alt+Tab、退出与重启；真实模型、普通用户、其余取消/错误/TUI/权限及视觉清单仍待执行）。
